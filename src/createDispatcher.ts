...
export const createDispatcher = (handler: () => void) => {
  const self = globalThis as typeof globalThis & { scheduler: IScheduler, setImmediate: (handler: () => void) => number };
  ...
  } else if ('setImmediate' in self && typeof self.setImmediate === 'function') {
    return () => self.setImmediate(() => handler());
  } else {
